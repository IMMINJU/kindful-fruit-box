...
export default function FruitBox() {
  ...
  const handleMouseDown = useCallback(
    ...
    [state.gameOver, getRelativePosition, updateSelection]
  );
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-24">
        ...
        {state.isSelecting && state.selectionStart && state.selectionEnd && (
          <>
            <div
              ...
              style={{
                left: Math.min(state.selectionStart.x, state.selectionEnd.x),
                top: Math.min(state.selectionStart.y, state.selectionEnd.y),
      ...
      <div className="mt-8 text-center">
        <p className={clsx(SCORE_FONT_SIZE, "mb-4")}>Score: {state.score}</p>
        {state.gameOver && (
          <p
            className={clsx(
              GAME_OVER_FONT_SIZE,
